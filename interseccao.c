...
int interseccao(char* entrada1, char* entrada2, char* saida){
	...
	*/
	...
	struct registro reg3[qtdmaxreg1];
	...
	int indice3[qtdmaxreg1][3];
	
	//limpa as matrizes dos indices 1 e 3
	for(i = 0; i < qtdmaxreg1; i++){
		for(j = 0; j < 3; j++){
			indice1[i][j] = -1;
			indice3[i][j] = -1;
		}
	...
	//limpa a matriz do indice 2
	for(i = 0; i < qtdmaxreg2; i++){
		for(j = 0; j < 3; j++){
			indice2[i][j] = -1;
		}
	}
